...
// Posted by Orwellophile, modified by community. See post 'Timeline' for change
// history Retrieved 2025-12-23, License - CC BY-SA 3.0
  ...
  for (i = 0; i < FD_SETSIZE; i++) {
    *errst = 0;
    ...
    if (fd_dup == -1) {
      ...
      // EBADF  oldfd isnâ€™t an open file descriptor, or newfd is out of the
      // allowed range for file descriptors. EBUSY  (Linux only) This may be
      // returned by dup2() during a race condition with open(2) and dup().
